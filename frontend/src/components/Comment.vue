<template>
  <div>
    <transition name="bounce">
      ...
      <v-card v-show="store.state.comment.toggle" class="comment-content" width="400">
        <v-row no-gutters>
          <v-col md="10">
            <v-card-title>
          ...
          <v-col md="2">
            ...
            <v-btn icon="mdi-close-circle-outline" @click="store.state.comment.toggle = false; removePulseLayer()"
              class="ma-2" variant="text" color="blue-lighten-2">
        ...
        <v-card-subtitle>
          ...
          erzeugt von: {{ store.state.comment?.commentData?.properties?.user_id }} <p>am:
            {{ store.state.comment?.commentData?.properties?.created_at }}
          </p>
        </v-card-subtitle>
        ...
        <div>
          <v-btn size="small" class="ma-2" variant="text" icon="mdi-thumb-up-outline"
          ...
          <v-btn size="small" class="ma-2" variant="text" icon="mdi-thumb-down-outline"
...
const snackbarPayload = ref("");
const connection = new WebSocket(import.meta.env.VITE_APP_WS_BASE_URL + "/comments/" + store.state.aoi.projectSpecification.project_id);
connection.addEventListener('message', ({ data }) => {
  snackbar.value = true;
  snackbarPayload.value = data;
