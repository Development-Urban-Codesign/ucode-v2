<template>
    <v-col cols="10" sm="1" style="position: absolute; right: 0; top: 60px;z-index:999">
        <v-select :items="['get', 'retrieve']" label="building" variant="outlined"
            @update:modelValue="sendBuildingRequest"></v-select>
        <v-select :items="['get', 'retrieve']" label="greenery" variant="outlined"
...
import { getbuildingsFromDB, getbuildingsFromOSM } from "../service/backend.service"
...
const sendGreeneryRequest = (e) => {
    e == "get" ? store.dispatch("aoi/getGreeneryFromOSM") : store.dispatch("aoi/getGreeneryFromDB")
}
</script>
